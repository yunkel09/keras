...
```{r}
autos_folds <- vfold_cv(autos_train, v = 2, strata = mpg)
```
...
Encontramos una adaptación del modelo neuralnets en el paquete {ParsnipExtra}
para que pudiera adaptarse al framework de tidymodels, sin embargo, después
de muchas pruebas fue muy complicado hacerlo funcionar. Las razones son las
siguientes:

1. El autor del paquete ParsnipExtra no siguió ciertas convenciones que hacen
...
demasiado como para que valga la pena probarlo.

2. Al intentar ajustar el modelo sin parámetros para tunear, se hace innecesario
realizar *resampling*, sin embargo, aun así no hay razón para usar el framework
de tidymodels. 

La mejor opción es utilizar neuralnet de forma nativa y probar con los
hiperparámetros que mejor se adaptan. Aun así, obtener la convergencia, dada la
cantidad de predictores se hace muy lenta.
...
```{r, echo=FALSE}
neural_res <- pin_read(board = tablero, name = "neural_res")
```

#### Ajuste del modelo

```{r}
mlp_neural <- mlp(hidden_units = 3, 
						epochs = 1) |>
...
```{r}
parametros <- mlp_param |> 
	update(epochs = epochs(c(100, 300)),
...
```{r}
neural_res <- neural_fit |> extract_fit_engine()
```
...
Lo primero que notamos es que obtenemos un *warning*

```{r}
collect_notes(neural_res)
```

`rsq()` es simplemente la correlación al cuadrado entre la verdad y la
estimación.

Debido a que `rsq()` calcula internamente una correlación, si la verdad o la
estimación son constantes, puede resultar en un error de división por cero. En
estos casos, se lanza una advertencia y se devuelve NA. Esto puede ocurrir
cuando un modelo predice un valor único para todas las muestras. Por ejemplo, un
modelo regularizado que elimine todos los predictores excepto el intercepto
haría esto. Otro ejemplo sería un modelo CART que no contiene divisiones.

Veamos cuantos modelos fueron ajustados:

```{r}
nrow(collect_metrics(neural_res, summarize = FALSE))
```
...
pin_write(board = tablero,
			 ...
			 name  = "neural_res",
			 type  = "rds",
			 title = "neuralnet sin parámetros",
			 description = "solo con fit, funcionó nítido, pero solo variables numéricas")
```
...
```{r, eval=FALSE, echo=FALSE}
pin_meta(tablero, "neural_res", 
			version = pin_versions(tablero, "neural_res")$version[[6]])$description
```


Ahora veamos de todos los modelos evaluados con cross-validación, cual de
todos nos dió el mejor resultado.
...
El número de épocas no parece tener un efecto en el rendimiento.

```{r}
show_best(neural_res) |> select(-.estimator)
```

Con base en estos resultados, tendría sentido realizar otra ejecución de
búsqueda en cuadrícula con valores más grandes de la penalización por caída de
peso.
...
```{r, echo=FALSE}
nnets_res <- pin_read(board = tablero, name = "nnets_res")
```
...
```{r}
tune_res <- nnets_res |> 
	bind_rows(as_workflow_set(rec_all_neural = neural_res))
```
